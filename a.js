...
const essayImageHosting = config.imageHosting.essayPath;

dirSync.rmdirSyncloop(essayImageHosting)
...
function converHtml(_postFile) {
  ...
  // 3.写入文件
  //tag 位置更改
  let tagReg = /<span class="_tags">.+<\/span>/
  fs.writeFileSync(outPutPath + '/' + 'context.html',T.replace(tagReg,'') + '<div class="essay-context">' + M + `</ol><div class="essay-hr"><div class="essay-end-tip">END</div><div class="essay-tag">${T.match(tagReg)}</div></div>
  <div id="mixxopost"></div>${F}`)
  ...
  dirSync.createDirs(essayImageHosting)

  const Sha1 = crypto.createHash('sha1')
  if (fs.existsSync(_imagePath)) {
    dirSync.loop(_imagePath, 'file').forEach(img => {
      if (img !== null) {   
        let readimg = fs.readFileSync(img, Buffer)
        let imgHash = require('crypto').createHash('sha1').update(readimg).digest('hex')
        let outImgPath = essayImageHosting + '/' + imgHash + '.' + img.split('/')[5].split(/\./).pop()
        fs.writeFileSync(outImgPath, readimg )
      }
    })
  }
}
...
function pages() {
  ...
  for (let i = 0; i < publicEssayFilePath.length; i++) {
    ...
    if (b.test(r)) {
      // 预览图
      let bannerName = r.match(b)[0].match(/\(.+\)/) !== null ? r.match(b)[0].match(/\(.+\)/)[0].slice(1,-1) : 'nosuchfile'

      li += `<div class="preview">
          <span class="preview-img" style="background-image:url(${bannerName})"></span>
          </div>`
        + '<span>'
        + r.match(s)[0] + '\r\n'
        //+ r.match(s)[1] + '\r\n'
...
function pushGallery() {
  ...
  // 复制
  const layout_galleryLinkId = /<%-galleryLinkId-%>/gm
  ...
  dir.forEach(formPath => {
    ...
    gallery_index += layout.replace(layout_galleryLinkId,galleryLinkCount)
      .replace(/<%-dataId-%>/gm,galleryLinkCount)
      .replace(layout_dataTag, info.tag)
