const fs = require('fs')
const imgHosting = JSON.parse(fs.readFileSync('./config.json', 'utf-8')).imageHosting.essay
/**
 * 
 * @param {*} fromTags 
...
function frontMatter(fromTags, imageTag) {
  ...
  let reg = [
    [/^banner:.*/m, ['<div class="_banner" style="background-image:url(', ');"></div>']],
    [/^title:.*/m, ['<h1 class="essay-title">', '</h1>']],
    [/^date:.*/m, ['<span class="_date">', '</span>']],
    [/^tags:.*/m, ['<span class="_tags"><em>#</em><span>', '</span></span>']],
  ...
  for (let i = 0; i < reg.length - 1; i++) {
    if (fromTags.match(reg[i][0]) !== null) {
      let t = fromTags.match(reg[i][0])[0].split(/:/)[1]
      let p = t + reg[i][1][1] + '\r\n'
      if (i == 0) {
        let bannerPath = './source/_post/_image/' + imageTag + '/' + t
        let bannerHash = fs.existsSync(bannerPath) ? require('crypto').createHash('sha1').update(fs.readFileSync(bannerPath, Buffer)).digest('hex') : ''
        m += reg[i][1][0] + imgHosting + bannerHash + '.' + t.split(/\./).pop() + reg[i][1][1] + '\r\n'
...
function cover(fromText, imageTag) {
  ...
  let loop = function (text) {
    ...
    let _toBlockQuote = function (e) {
      let  quote= []
      let b = /%-quotebr-%>.+?>%-quotebr-%/gm
      result = e.replace(/\r\n|\r|\n/gm, '%-quotebr-%')
      
      if (blockquote.test(result) && b.test(result)) {   
        result.match(b).forEach(e => {
    
          quote.push(e.replace(/%-br-%/gm, '<br>').replace(/%-quotebr-%>/gm, ''))
        })
        result = result.replace(b, '<%-quote-%>')
        quote.forEach(e => {
          result = result.replace(/<%-quote-%>/m, `<blockquote>${e}</blockquote>`)
        })
      }
      result = result.replace(/%-quotebr-%/gm,'\r\n')
    }
    _toBlockQuote(text)
    ...
    let _toImg = function (e) {
      if (img.test(e)) {
        ...
        let imgPath = './source/_post/_image/' + imageTag + '/' + i[1].slice(0, -1)
        let imgBuffer = fs.readFileSync(imgPath, Buffer)
        let imgHash = fs.existsSync(imgPath) ? require('crypto').createHash('sha1').update(imgBuffer).digest('hex') : 'nothisfile'
        result = e.replace(img, `<span class="essay-img" img-alt="${i[0].slice(2)}"><img src="${imgHosting + imgHash}\.${i[1].slice(0, -1).split(/\./).pop()}"></span>`)
